...
bi_settings.CNC_work_time_1 = 455   #int(input("请输入CNC的工作时间："))
bi_settings.CNC_work_time_2 = 182
...
def Optimal_Path(ai_settings):
    ...
    for one_step in range(1, 9):
        ...
        if bi_settings.CNC_type[one_step-1] == 2:
            if one_step < 3 and one_step > 0:
                ...
                L1 = 4
            [T1,F_CNC_1, Take_time_1, Give_time_1] = fun.oneStep_time(bi_settings, one_step)
            #print('****',T1)
            fun.update_status(bi_settings, L1, one_step, T1, F_CNC_1)
            ...
            for two_step in range(1, 9):
                ...
                #print(bi_settings.Locate_RGV,bi_settings.CNC_remaining_processing_time)
                if bi_settings.CNC_type[two_step-1] == 2:
                    [T2,F_CNC_2, Take_time_2, Give_time_2] = fun.oneStep_time(bi_settings, two_step)
                    T = T1 + T2
                    #print('*****',T)
                    if T < min:
                        min = T
                        ...
                        RGV_L = L1
                        F_CNC = F_CNC_1
                        Take_time = Take_time_1
                        Give_time = Give_time_1
    #print('*' ,one_step_out, RGV_L, T1_out)
    return [T1_out, one_step_out, RGV_L, F_CNC, Take_time, Give_time]
